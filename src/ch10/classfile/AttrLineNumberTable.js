...
class LineNumberTableAttribute extends AttributeInfo {
    ...
    get_line_number(pc) {
        for (let i = this.lineNumberTable.length - 1; i >= 0; i--) {
            let entry = this.lineNumberTable[i];
            if (pc >= parseInt(entry.start_pc)) {
                return parseInt(entry.line_number);
            }
        }
        return -1;
    }
